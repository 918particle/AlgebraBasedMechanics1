...
\begin{itemize}
\item \textit{Derivative} of sine: $\frac{d}{dx} \sin(kx) = k\cos(kx)$
\item \textit{Derivative} of cosine: $\frac{d}{dx} \cos(kx) = -k\sin(kx)$
\end{itemize}
...
The \textbf{gravitational potential energy} is 
...
\textbf{It turns out that the solution Equation \ref{eq:2}} is a sum of sines and cosines.
...
\begin{enumerate}
\item (a) Create a graph below of the \textit{period} of of the pendulum versus length, and show that the period is proportional to the square root of the length. (b) Reproduce the results with a real pendulum in the lab. (c) Discuss any differences you observe between the real and simulated data.  \\ \vspace{6cm}
