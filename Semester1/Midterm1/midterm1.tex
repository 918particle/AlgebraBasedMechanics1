...
\item A system moves according to $x(t) = 12t - 3t^2$ m. (a) What is the average velocity between these times? (b) What is the acceleration? \\ \vspace{3cm}
...
\begin{enumerate}
\item What is the terminal velocity of a 60 kg skydiver with area $A = 0.25$ m$^2$, and drag coefficient $C = 0.5$?  Use the standard density of air: $\rho = 1.2$ kg m$^{-3}$. (b) What is the terminal velocity if she opens the parachute, increasing the cross-sectional area by a factor of 100? \\ \vspace{4cm}
\item Calculate the change in length of the upper leg bone (the femur) when a 55.0 kg woman supports 45.0 kg of her mass on it, assuming the bone to be equivalent to a uniform rod that is 30.0 cm long and 2.00 cm in radius. Young's Modulus for bone is $9 \times 10^9$ N m$^{-2}$. \\ \vspace{4cm}
\item Find the mass of a picture hanging from a steel nail, given that the nail bends only 4.5 microns ($10^{-6}$ m).  The shear modulus is $80 \times 10^9$ N m$^{-2}$, and the diameter of the nail is 3.00 mm. \\ \vspace{4cm}
\item \textbf{Scaling Problem:} Consider how we finished the exercise on the bone and Young's modulus.  Scale this exercise to apply to a steel beam supporting a building.  The beam is 20 meters long, has a radius of 10 cm, and supports $\approx 10^4$ kg.  What is the change in length?
\end{enumerate}
