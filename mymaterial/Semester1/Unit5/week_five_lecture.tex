...
\title{Algebra-Based Physics-1: Mechanics (PHYS135A-01): Unit 5}
\date{\today}
...
Suppose a force $F$ is \textit{applied} to a system of length $L$ and cross-sectional area $A$, which compresses an amount $x$.  Let \textit{Young's Modulus} be defined by $Y = k \frac{A}{L}$.  Rearranging Hooke's Law  gives \\
\begin{equation}
